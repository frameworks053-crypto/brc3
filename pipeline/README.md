...
**목록에 없는 컴프는 아예 건드리지 않습니다.** 인트로·아웃트로처럼 곡이 아닌
컴프가 메인에 섞여 있으면 그냥 빼두세요. 이미지도, 타이밍도, 레이어 순서도
손으로 맞춰둔 그대로 유지됩니다.
...
```
파일명 앞 번호        트랙 번호       slot_comps 순서       곡 경계 순서
01-Neon Rain.png  →   1번 곡    →   "Change - Things"  →  첫 번째 경계
02-Late Sun.png   →   2번 곡    →   "Change - Things 2" → 두 번째 경계
   …                    …               …                     …
```

`plpipe ingest` 가 파일명 앞의 번호를 읽어 트랙에 배정하고, `plpipe status` 로
어떤 이미지가 몇 번 곡에 붙었는지 확인할 수 있습니다. 번호가 없는 파일은
이름순으로 빈 자리를 채웁니다.
